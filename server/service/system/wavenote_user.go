...
func (wavenoteUserService *WavenoteUserService) GetWavenoteUserInfoList(ctx context.Context, info systemReq.WavenoteUserSearch) (list []system.WavenoteUser, total int64, err error) {
	...
	if info.ThirdAccount != nil && *info.ThirdAccount != "" {
		db = db.Where("third_account LIKE ?", "%"+*info.ThirdAccount+"%")
	}
	if info.ChannelUid != nil && *info.ChannelUid != "" {
		db = db.Where("channel_uid LIKE ?", "%"+*info.ChannelUid+"%")
	}
	if info.Origin != nil {
		db = db.Where("origin = ?", *info.Origin)
	}
	if info.SyncStatus != nil {
		db = db.Where("sync_status = ?", *info.SyncStatus)
	}
